 ...
 * IOException { final String requestTokenHeader =
 * request.getHeader("Authorization"); String username = null; String jwtToken =
 * null; // JWT Token is in the form "Bearer token". Remove Bearer word and get
 * // only the Token if (requestTokenHeader != null &&
 ...
 * logger.warn("JWT Token does not begin with Bearer String"); } // Once we get
 * the token validate it. if (username != null &&
 * SecurityContextHolder.getContext().getAuthentication() == null) { UserDetails
 * userDetails = this.jwtUserDetailsService.loadUserByUsername(username); // if
 * token is valid configure Spring Security to manually set // authentication if
